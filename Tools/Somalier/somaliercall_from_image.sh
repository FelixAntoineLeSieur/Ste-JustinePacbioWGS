...
usage() { echo "Usage: $0 [-i <familyID>] [-p <proband_name>] [-m <mother_name>] [-f <father_name>] [-r <fasta_path>]" 1>&2; exit 1; }

while getopts ":p:m:f:i:r:d:" o; do
    case "${o}" in
        p)
            proband_name=${OPTARG}
            ;;
...
#There is a 1-kg tar to extract to use this command
...
apptainer exec -C -B $HOME --pwd $input_directory \
  ...
  --sample-prefix "$1" \
  $1.$family_id.normed.joint.GRCh38.small_variants.phased.vcf.gz
}


#Get Required files if not present
if [ ! -f "$here_folder/sites.hg38.vcf.gz" ]; then
wget https://github.com/brentp/somalier/files/3412456/sites.hg38.vcf.gz -O $here_folder/../Tools/Somalier/sites.hg38.vcf.gz
fi
echo "Running Somalier extract on:"
echo $proband_name

apptainer_extract "$proband_name" "$here_folder/sites.hg38.vcf.gz"
echo $mother_name
apptainer_extract "$mother_name" "$here_folder/sites.hg38.vcf.gz"
if [ ! -z "$father_name" ]; then
	echo $father_name
	apptainer_extract "$father_name" "$here_folder/sites.hg38.vcf.gz"
fi
